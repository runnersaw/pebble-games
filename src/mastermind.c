...
static Window *s_mastermind_window;
static Layer *s_mastermind_layer;
...
static void add_guess(short color) {
  *(current_guess+current_guess_no_chosen) = color;
  ...
  layer_mark_dirty(s_mastermind_layer);
}
...
static void draw_mastermind(Layer *layer, GContext *ctx) {
...
static void back_handler() {
  if (current_guess_no_chosen == 0) {
    window_stack_pop(s_mastermind_window);
  } else {
    remove_guess();
  }
  layer_mark_dirty(s_mastermind_layer);
...
static void mastermind_load(Window *window) {
  s_mastermind_layer = layer_create(GRect(0,0,144,152));
  layer_set_update_proc(s_mastermind_layer, draw_mastermind);
  layer_add_child(window_get_root_layer(window), s_mastermind_layer);
...
static void mastermind_unload(Window *window) {
  layer_destroy(s_mastermind_layer);

  free(target);
  free(guesses);
  free(current_guess);
  
  window_destroy(s_mastermind_window);
}
...
void mastermind_init() {
  s_mastermind_window = window_create();
  
  window_set_click_config_provider(s_mastermind_window, click_config_provider);
  window_set_window_handlers(s_mastermind_window, (WindowHandlers) {
    .load = mastermind_load,
    .unload = mastermind_unload,
  });
  
  const bool animated = true;
  window_stack_push(s_mastermind_window, animated);
}
#endif